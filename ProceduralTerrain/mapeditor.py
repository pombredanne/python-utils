...
class MapEditor():
	...
	def onMouseTask(self):
		...
		# for this small example I will traverse everything, for bigger projects this is probably a bad idea
	...
	def setupMouseCollision(self):
		...
		# like any other collision detection system with a traverser and a handler
		...
		#self.mPickRay.setDirect(render.getRelativeVector(base.camera, Vec3(0,1,0)))
		
		# create our collision Node to hold the ray
		self.mPickNode = CollisionNode('pickRay')
		self.mPickNode.addSolid(self.mPickRay)
		
		# Attach that node to the camera since the ray will need to be position
		# relative to it, returns a new nodepath
		# we'll use the default geometry mask
		# this is inefficient but its for mouse picking only
