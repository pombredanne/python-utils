...
class Walker(NodePath):
	...
	def turnBody(self, desiredHeading, elapsed):
		...
		# Make sure we are turning the quick way.  If we're turning more than
		# 180 we should be turning the other direction
		if deltaH > 180.0:
			heading += 360.0
			deltaH = desiredHeading - heading
		elif deltaH < -180.0:
	...
	def move(self, desiredVelocity, desiredHeading, elapsed):
		# save characters initial position so that we can restore it,
